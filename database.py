...
DB_PATH = 'students.db'
CSV_PATH = 'Data.csv' # Укажите путь к вашему CSV файлу
...
def load_data_from_csv_to_db():
    ...
    try:
        ...
        for index, row in df.iterrows():
            name = row['ФИО'] # Теперь точно 'ФИО'
            age_str = str(row['Возраст']).replace(',', '.') # ИСПРАВЛЕНО: Заменяем запятую на точку
            try:
                age = float(age_str)
            except ValueError:
            ...
            if pd.notna(games_str) and games_str.strip() != '':
                games_list = [g.strip() for g in games_str.split(',')] # Разделяем по запятой
                for game_name in games_list:
                    if game_name: # Проверяем, что имя игры не пустое
                        ...
                        if game_id:
                            ...
                            # Используем текущую дату или дату из CSV, если она есть
...
def add_game(game_name):
    ...
    try:
        ...
        if game_id:
            ...
            # Если не вставилось (хотя OR IGNORE должен предотвратить дубликаты), что-то пошло не так
...
def get_student_data_for_model(student_id):
    ...
    # Получаем список игр, которые уже играет ученик (из назначений)
    cursor.execute("""
        SELECT g.name
        FROM assignments a
        JOIN games g ON a.game_id = g.id
